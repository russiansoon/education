## MAIN

- [The "data" URL scheme](http://tools.ietf.org/html/rfc2397)
- [Data URI scheme](https://en.wikipedia.org/wiki/Data_URI_scheme)
- [ASCII](https://en.wikipedia.org/wiki/ASCII)
- [Best Practices for Speeding Up Your Web Site](https://developer.yahoo.com/performance/rules.html#num_http=)
- [CDN](https://en.wikipedia.org/wiki/Content_delivery_network)
- [Browser Cache Usage](http://yuiblog.com/blog/2007/01/04/performance-research-part-2/)
- [ImageMagick: Create favicon](http://www.imagemagick.org/script/index.php)
- [data:urls – what are they and how to use them](http://www.phpied.com/data-urls-what-are-they-and-how-to-use/)
- [Source Map Revision 3 Proposal](https://docs.google.com/document/d/1U1RGAehQwRypUTovF1KRlpiOFze0b-_2gc6fAH0KY0k/edit)
- [DIY source maps](http://www.phpied.com/diy-source-maps/)
- [When to Base64 Encode Images (and When Not To)](http://davidbcalhoun.com/2011/when-to-base64-encode-images-and-when-not-to/)
- [Client Data Management and Caching](https://msdn.microsoft.com/en-us/library/hh404101.aspx)
- [Using media queries](https://developer.mozilla.org/en-US/docs/Web/CSS/Media_Queries/Using_media_queries)
- [JavaScript Design Patterns: Singleton](http://robdodson.me/javascript-design-patterns-singleton/)
- [Формат JSON, метод toJSON](https://learn.javascript.ru/json#формат-json)

## REST

- [REST (русская википедиа)](https://ru.wikipedia.org/wiki/REST)
- [**Representational State Transfer (REST)**](http://www.ics.uci.edu/~fielding/pubs/dissertation/rest_arch_style.htm)
- [**Dissertation by Roy Thomas Fielding (2000): Architectural Styles and the Design of Network-based Software Architectures**](http://www.ics.uci.edu/~fielding/pubs/dissertation/top.htm)
- [What is REST? (video)](http://www.restapitutorial.com/lessons/whatisrest.html)

## GULP / BOWER

- [Setting up Gulp, Bower, Bootstrap Sass & FontAwesome](http://ericlbarnes.com/setting-gulp-bower-bootstrap-sass-fontawesome/)
- 